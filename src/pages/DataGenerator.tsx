...
export default function Home() {
  ...
  useEffect(() => {
    const updateClock = () => {
      ...
      setCurrentEAT(`${String(eatHour).padStart(2, "0")}:${String(eatMin).padStart(2, "0")}`);
    };
   ...
   * mutated with setDate/setHours and formatted locally — silently moving every
   * requested EAT window on non-UTC clients.
   */
  ...
  const addLog = (msg: string) => {
    console.log(msg);
    setConsoleLogs((prev) => [...prev.slice(-500), `${new Date().toLocaleTimeString()}: ${msg}`]);
  };
...
<svg width="1920" height="1080" viewBox="0 0 1920 1080" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="960" y="600" font-size="24" fill="#808080" text-anchor="middle">${timeframe.toUpperCase()}</text>
    ...
    const widthMultiplier = timeframe === "4h" ? 0.58 : timeframe === "1h" ? 0.68 : 0.72;
...
<svg width="1920" height="1080" viewBox="0 0 1920 1080" xmlns="http://www.w3.org/2000/svg">
  ...
  const handleDownload = async (csvOverride?: string | null) => {
    ...
    const chartFiles = filesSnapshot.filter(
      (file) =>
        ["4h", "1h", "30m"].includes(file.type) && !!file.candleData && file.candleData.length > 0,
    );
  ...
  const fetchOhlcData = async (): Promise<string | null> => {
    try {
      ...
      return null;
    } // preserved: previous contract returned null on failure
  };
  ...
  const fetchCandleData = async (
    ...
    while (rateLimitAttempts <= MAX_RATE_LIMIT_RETRIES) {
      try {
        ...
        addLog(`🔄 API Req [EAT]: ${interval} ${fromDate} -> ${toDate}`);
        ...
        const cleanedCandles = removeRepeatedFlatlineArtifacts(orderedCandles, intervalMinutes);
  ...
  const executeGeneration = async (
    ...
    for (const { tf, label, lookbackHours } of timeframes) {
      ...
      for (let dayOffset = 0; dayOffset < daysSpan; dayOffset++) {
        ...
        while (currentTargetDate.getDay() === 0 || currentTargetDate.getDay() === 6) {
        ...
        if (generatedTargetDates.has(targetDateKey)) {
          addLog(`⏭️ Weekend maps to already generated trading day: ${targetDateKey}`);
          continue;
        }
        ...
        const fileExists = generatedFilesRef.current.some(
          (name) =>
            name.includes(expectedFileName) &&
            /(^|[^0-9])Day(\d+)/.exec(name)?.[2] === String(dayOffset),
        );
        ...
        const chartStartDate = new Date(chartEndDate.getTime());
        ...
        try {
          ...
          addLog(`❌ Error for ${label} - Day ${dayOffset + 1}/${daysSpan}: ${String(error)}`);
  ...
  return (
    <div className="app-shell min-h-screen bg-background text-foreground flex flex-col font-sans">
      ...
      {/* ═══ MAIN LAYOUT ═══ */}
      <main className="page-frame flex-1 w-full px-4 py-4 sm:px-6 sm:py-6 lg:grid lg:grid-cols-12 gap-4 sm:gap-5">
        {/* ─── SIDEBAR ─── */}
        <aside className="lg:col-span-4 space-y-3">
          {/* ── CHART SCREENSHOTS BLOCK ── */}
          <div className="panel rounded-xl border border-white/6 bg-[#0d0608] overflow-hidden">
            {/* Block title bar */}
            <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between bg-primary/8">
              <div>
                ...
                <p className="text-[11px] text-white/35 mt-1 font-mono">PNG · 4H · 1H · 30M</p>
              </div>
                    ...
                    >
                      <span className="flex items-center gap-2">
                        {symbol ? (
                          ...
                          <span className="text-white/30 font-normal">Select symbol…</span>
                        )}
                      </span>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2.5 cursor-pointer select-none group">
                  ...
                  <span className="text-[11px] font-semibold text-white/50 group-hover:text-white/80 transition-colors">
                    Custom end time <span className="font-mono text-white/25">(EAT)</span>
                  </span>
                  {!useCustomEndTime && (
                    <span className="ml-auto text-[10px] font-mono text-primary/60">→ Latest</span>
                  )}
                </label>
                {useCustomEndTime && (
                  <Input
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2.5 cursor-pointer select-none group">
                  ...
                  />
                  <span className="text-[11px] font-semibold text-white/50 group-hover:text-white/80 transition-colors">
                    Specify time range <span className="font-mono text-white/25">(EAT)</span>
                  </span>
                </label>
                {ohlcSpecifyTime && (
          ...
          {/* ── DOWNLOAD ── */}
          <div className="panel rounded-xl border border-white/6 bg-[#0d0608] p-5 space-y-4">
            <p className="text-[10px] font-bold text-white/35 uppercase tracking-[0.15em]">
              Include in Download
            </p>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-white/25 font-mono mt-1">PNG · 4H 1H 30M</p>
                </div>
              </label>
              <label
                className={cn(
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-white/25 font-mono mt-1">30M · EAT</p>
                </div>
              </label>
            </div>
        ...
        <div className="lg:col-span-8 space-y-4">
                  ...
                  <div className="flex items-start justify-between relative z-10">
                    ...
                    <span
                      className={cn(
                        ...
                        !isActive && !isDone && "text-white/30 border-white/10 bg-white/3",
                        isActive && "text-primary border-primary/40 bg-primary/12",
                        isDone && "text-emerald-400 border-emerald-500/30 bg-emerald-500/10",
                      )}
                    >
                      {s.status === "idle" ? "Ready" : s.status === "running" ? "Active" : "Done"}
                    </span>
                  </div>
          ...
          <div
            className="panel rounded-xl border border-white/6 bg-[#0d0608] overflow-hidden flex flex-col"
            style={{
          ...
          >
            <div className="px-5 py-3.5 border-b border-white/5 bg-black/20 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-black text-white leading-none">Generated Assets</h2>
                  <p className="text-[10px] text-white/25 font-mono mt-0.5">Output directory</p>
                </div>
              </div>
              <Button
                size="sm"
                onClick={() => handleDownload()}
                disabled={generatedFiles.length === 0 || isGenerating || isDownloading}
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          !isError && !isSuccess && !isWarning && "text-white/30",
                        )}
                      >
