/**
 * Server-only implementation for the verifier. Kept out of *.functions.ts so the
 * server-function splitting transform cannot drop these runtime helpers.
 */
...
export async function runVerification(input: {
  ...
  const attempts: {
  ...
  }[] = [
    {
      ...
      missingKeyLabel: "LOVABLE_API_KEY is not configured",
    },
    {
      provider: "openrouter",
      ...
      missingKeyLabel: "OPENROUTER_API_KEY is not configured",
    },
    {
      provider: "nvidia",
      ...
      missingKeyLabel: "NVIDIA_API_KEY is not configured",
    },
  ];

  for (const attempt of attempts) {
    if (!attempt.key) {
      warnings.push(attempt.missingKeyLabel);
      continue;
    }
    for (const model of attempt.models) {
      try {
        ...
        return { verdict, provider: attempt.provider, model, warnings };
      } catch (error) {
        warnings.push(
          `${attempt.provider} ${model} failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }

  throw new Error(`Verifier unavailable. ${warnings.join(" | ")}`);
}
