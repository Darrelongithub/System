...
function emaClose(ctx: AnalysisContext, i: number, n: number): number | undefined {
  let byN = emaCache.get(ctx);
  if (!byN) {
    byN = new Map();
    emaCache.set(ctx, byN);
  }
...
function rangeHL(ctx: AnalysisContext, from: number, to: number) {
  ...
  for (let j = from; j <= to; j++) {
    const x = c(ctx, j);
    if (x.invalid || x.high === undefined || x.low === undefined) return undefined;
...
export const dualThrustSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    ...
    if (i < P.dualThrustN + 5) return fail("w");
    const x = c(ctx, i);
    if (x.close === undefined || x.open === undefined) return fail("o");
    let HH = -Infinity,
      LC = Infinity,
      HC = -Infinity,
      LL = Infinity;
    for (let j = i - P.dualThrustN; j < i; j++) {
      const b = c(ctx, j);
      if (b.invalid || b.high === undefined || b.low === undefined || b.close === undefined)
        return fail("r");
      if (b.high > HH) HH = b.high;
      if (b.close < LC) LC = b.close;
      if (b.close > HC) HC = b.close;
      if (b.low < LL) LL = b.low;
    }
    const range = Math.max(HH - LC, HC - LL);
    if (!(range > 0)) return fail("f");
    const atr = atr14(ctx, i - 1);
    if (!atr) return fail("a");
    ...
    if (x.close > x.open + P.dualThrustK * range) {
      ...
      return pass(
        ...
        x.close,
        sl,
        tpR(x.close, sl, "long", P.tpMultiple),
        "long",
        "market",
        day + ":L",
      );
    ...
    if (x.close < x.open - P.dualThrustK * range) {
      ...
      return pass(
        ...
        x.close,
        sl,
        tpR(x.close, sl, "short", P.tpMultiple),
        "short",
        "market",
        day + ":S",
      );
...
export const macdCrossSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    const P = FINAL_PARAMS;
    if (i < 40) return fail("w");
    const x = c(ctx, i);
    if (x.close === undefined) return fail("c");
    const macdAt = (end: number) => {
      const f = emaClose(ctx, end, P.macdFast),
        s = emaClose(ctx, end, P.macdSlow);
      return f !== undefined && s !== undefined ? f - s : undefined;
    };
    const macds: number[] = [],
      macdsPrev: number[] = [];
    for (let j = i - (P.macdSig - 1); j <= i; j++) {
      const m = macdAt(j);
      if (m === undefined) return fail("m");
      macds.push(m);
    }
    for (let j = i - P.macdSig; j <= i - 1; j++) {
      const m = macdAt(j);
      if (m === undefined) return fail("m");
      macdsPrev.push(m);
    }
    ...
    const mNow = macds[macds.length - 1]!,
      mPrev = macdsPrev[macdsPrev.length - 1]!;
    const atr = atr14(ctx, i - 1);
    if (!atr) return fail("a");
    const day = eatDay(x.datetime);
...
export const pdhRetestSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    ...
    if (brokeH && x.low <= prior.high && x.close > prior.high) {
      ...
      const sl = prior.high - P.pdhAtrMult * atr;
      if (!(sl < x.close)) return fail("sl");
      return pass(
        "PDH retest long",
        x.close,
        sl,
    ...
    if (brokeL && x.high >= prior.low && x.close < prior.low) {
      ...
      const sl = prior.low + P.pdhAtrMult * atr;
      if (!(sl > x.close)) return fail("sl");
      return pass(
        "PDL retest short",
        x.close,
...
export const williamsRFadeSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    const P = FINAL_PARAMS;
    if (i < 20) return fail("w");
    const x = c(ctx, i);
    if (x.close === undefined) return fail("c");
    const n = P.williamsPeriod;
    const band = rangeHL(ctx, i - (n - 1), i);
    if (!band) return fail("b");
    const den = band.high - band.low;
    if (den <= 0) return fail("f");
    const wr = ((band.high - x.close) / den) * -100;
    const bandP = rangeHL(ctx, i - n, i - 1);
    if (!bandP || c(ctx, i - 1).close === undefined) return fail("p");
    const denP = bandP.high - bandP.low;
    if (denP <= 0) return fail("f2");
    ...
    if (wrP <= -80 && wr > -80) {
      ...
      return pass(
        ...
        x.close,
        sl,
        tpR(x.close, sl, "long", P.williamsTp),
        "long",
        "market",
        day + ":L",
      );
    ...
    if (wrP >= -20 && wr < -20) {
      ...
      return pass(
        ...
        x.close,
        sl,
        tpR(x.close, sl, "short", P.williamsTp),
        "short",
        "market",
        day + ":S",
      );
...
export const threeSoldiersSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    const P = FINAL_PARAMS;
    if (i < 15) return fail("w");
    const a = c(ctx, i - 2),
      b = c(ctx, i - 1),
      x = c(ctx, i);
    if (
      [a, b, x].some((candle) => candle.invalid) ||
      [a.open, a.close, b.open, b.close, x.open, x.close].some((v) => v === undefined)
    )
      return fail("o");
    const atr = atr14(ctx, i - 1);
    if (!atr) return fail("a");
    ...
    if (bull) {
      ...
      const sl = Math.min(a.low ?? x.close!, x.close! - atr);
      if (!(sl < x.close!)) return fail("sl");
      return pass(
        "3S",
        x.close!,
        sl,
        tpR(x.close!, sl, "long", P.soldiersTp),
        "long",
        "market",
        day + ":L",
      );
    ...
    if (bear) {
      ...
      const sl = Math.max(a.high ?? x.close!, x.close! + atr);
      if (!(sl > x.close!)) return fail("sl");
      return pass(
        "3C",
        x.close!,
        sl,
        tpR(x.close!, sl, "short", P.soldiersTp),
        "short",
        "market",
        day + ":S",
      );
...
export const morningStarSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    ...
    if (
      [a, b, x].some((candle) => candle.invalid) ||
      [a.open, a.close, b.open, b.close, x.open, x.close, a.high, a.low].some(
        (v) => v === undefined,
      )
    )
      return fail("o");
    ...
    if (morning) {
      ...
      const sl = Math.min(a.low!, b.low ?? a.low!) - 0.1 * atr;
      if (!(sl < x.close!)) return fail("sl");
      return pass(
        "MS",
        x.close!,
        sl,
        tpR(x.close!, sl, "long", P.starTp),
        "long",
        "market",
        day + ":L",
      );
    ...
    if (evening) {
      ...
      const sl = Math.max(a.high!, b.high ?? a.high!) + 0.1 * atr;
      if (!(sl > x.close!)) return fail("sl");
      return pass(
        "ES",
        x.close!,
        sl,
        tpR(x.close!, sl, "short", P.starTp),
        "short",
        "market",
        day + ":S",
      );
...
export const classicPivotSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    ...
    if (x.low <= s1 && x.close > s1 && x.close < pp) {
      ...
      const sl = s1 - P.pivotAtr * atr;
      if (!(sl < x.close)) return fail("sl");
      return pass(
        "S1",
        x.close,
        sl,
        tpR(x.close, sl, "long", P.pivotTp),
        "long",
        "market",
        day + ":L",
      );
    ...
    if (x.high >= r1 && x.close < r1 && x.close > pp) {
      ...
      const sl = r1 + P.pivotAtr * atr;
      if (!(sl > x.close)) return fail("sl");
      return pass(
        "R1",
        x.close,
        sl,
        tpR(x.close, sl, "short", P.pivotTp),
        "short",
        "market",
        day + ":S",
      );
...
export const ichimokuTkSpec: StrategyCheck = {
  ...
  run(ctx, i) {
    const P = FINAL_PARAMS;
    if (i < 60) return fail("w");
    const x = c(ctx, i);
    if (x.close === undefined) return fail("c");
    const mid = (from: number, to: number) => {
      const r = rangeHL(ctx, from, to);
      return r ? (r.high + r.low) / 2 : undefined;
    };
    const tenkan = mid(i - (P.ichiTenkan - 1), i),
      kijun = mid(i - (P.ichiKijun - 1), i);
    const tenkanP = mid(i - P.ichiTenkan, i - 1),
      kijunP = mid(i - P.ichiKijun, i - 1);
    ...
    if (tenkanP <= kijunP && tenkan > kijun && x.close > cloudTop) {
      ...
      const sl = Math.min(kijun, x.close - 1.5 * atr);
      if (!(sl < x.close)) return fail("sl");
      return pass(
        "Ichi L",
        x.close,
        sl,
        tpR(x.close, sl, "long", P.ichiTp),
        "long",
        "market",
        day + ":L",
      );
    ...
    if (tenkanP >= kijunP && tenkan < kijun && x.close < cloudBot) {
      ...
      const sl = Math.max(kijun, x.close + 1.5 * atr);
      if (!(sl > x.close)) return fail("sl");
      return pass(
        "Ichi S",
        x.close,
        sl,
        tpR(x.close, sl, "short", P.ichiTp),
        "short",
        "market",
        day + ":S",
      );
...
export const donchian55Spec: StrategyCheck = {
  ...
  run(ctx, i) {
    const P = FINAL_PARAMS;
    if (i < P.donchianN) return fail("w");
    const x = c(ctx, i);
    if (x.close === undefined) return fail("c");
    const prior = rangeHL(ctx, i - P.donchianN, i - 1);
    const stop = rangeHL(ctx, i - P.donchianStopN, i - 1);
    if (!prior || !stop) return fail("r");
    const atr = atr14(ctx, i - 1);
    if (!atr) return fail("a");
    const day = eatDay(x.datetime);
    if (x.close > prior.high) {
      if (isConsumed(ctx, "donchian-55", day + ":L")) return fail("d");
      const sl = Math.max(stop.low, x.close - 2 * atr);
      if (!(sl < x.close)) return fail("sl");
      return pass(
        "D55 L",
        x.close,
        sl,
        tpR(x.close, sl, "long", P.donchianTp),
        "long",
        "market",
        day + ":L",
      );
    ...
    if (x.close < prior.low) {
      ...
      const sl = Math.min(stop.high, x.close + 2 * atr);
      if (!(sl > x.close)) return fail("sl");
      return pass(
        "D55 S",
        x.close,
        sl,
        tpR(x.close, sl, "short", P.donchianTp),
        "short",
        "market",
        day + ":S",
      );
